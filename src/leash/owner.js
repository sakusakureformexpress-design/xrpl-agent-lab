...
export class LeashOwner {
   ...
   * 予算枠を閉じる予約をする。
   *
   * 送金元が出す tfClose は即時に閉じない。チャネルに残高があれば
   * SettleDelay 秒後に閉じる予約になり、その間は受取人が未換金の伝票を換金できる
   * （＝エージェントは予約後も上限までは使える）。残額は閉じた時点で持ち主に戻る。
   * 守っているのは上限そのものであり、この操作は事後処理である（ADR 0007）。
   */
