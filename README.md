...
> **仕組みを触って確かめたいなら** [`demo/explainer.html`](demo/explainer.html) を手元のブラウザで開く
> （GitHub 上では生の HTML に見える）。

エージェントはプロンプトインジェクションで乗っ取られる。
XRPL 上のエージェント向けの予算管理は、どれもエージェントと同じホストの上にあるため、
乗っ取られた時点で制限も一緒に無効化されうる。

XRPL Leash は上限と支払先を**台帳の検証ルール**に置く。
Payment Channel の経路では、エージェントが乗っ取られても上限超過と未承認先への送金は成立しない。
（x402 のように支払先がその場で決まる経路はブローカーが担い、こちらはソフトウェアによる強制。被害上限はブローカー口座の残高）
...
# 枠を閉じる予約をする。SettleDelay（既定1時間）後に閉じ、残額は持ち主に戻る
# ※ 即時停止ではない。守っているのは上限そのものであり、閉じる操作は事後処理
...
いま何本の枠が開いていて、それぞれいくら残っていて、**その上限を誰が強制しているのか**を
1画面で見る。鍵は要らない。

```bash
# 読み取り専用のダッシュボード（127.0.0.1 のみ）
...
- **Tier 1（台帳が強制）** — 枠ごとの上限・使用・残り・期限。深刻度はメーターの色と記号の両方で示す
- **Tier 2（ブローカーが強制）** — エージェントごとの当日上限・新規支払先の枠・許可/拒否の設定
- **台帳に残った記録** — `SourceTag` で本ツール経由の取引だけを抽出。**拒否された取引も出る**

設計上の制約（意図的なもの）:

| 制約 | 理由 |
|---|---|
| **鍵を持たない** | 起動に要るのは公開アドレスとポリシーのパスだけ。シードは読まない |
| **書き込む口が無い** | `GET` 以外は 405。枠を閉じる操作は CLI（`revoke`）に残してある |
| **127.0.0.1 のみ** | 認証を持たない。外部に公開する前提の作りではない |
| **`Host` ヘッダを検証** | これが無いと DNS リバインディングで、外部のページが利用者のブラウザ経由でこの API を読める |
| **他人のメモを描画しない** | 台帳のメモ欄は誰でも書ける。自分たちの `MemoType` のものだけを構造化して出す |

ヒーローの数値は「**いまエージェント側が動かせる上限の合計**」であり、
台帳が強制している分とソフトウェアが強制している分を**分けて**表示する。
両者は侵害されたときの挙動が違うため、合算した数字だけを見せるのは誤解を招く。
...
| `docs/` | 設計判断・仕様の提案・セキュリティレビュー・作業ログ |
...
独立したレビュー（別セッションの AI エージェントによる）の結果と全指摘への対応は [docs/91-security-review.md](docs/91-security-review.md)。
