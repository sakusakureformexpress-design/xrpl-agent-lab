...
前提: 本リポジトリは既に公開済みであり、第三者がコードを読む。
...
- **影響**: これは `examples/` だが、リポジトリが提示する x402 の**参照実装**である。
  読む人はここを読み、実装者はここを写す。放置すれば merchant 側の資金（＝売上）が失われる。
...
- ~~**LOW-12** 検証スクリプト `src/verify/05-x402.js` が `/tmp` の固定パスを使う。~~
  **2026-09-24 対応済み。** 実行ごとに `fs.mkdtempSync` で本人専用の一時ディレクトリを作る

## 現在の状態

- `npm test` **87/87 合格**（2026-09-18 時点。2026-09-23 現在は 129/129）
